...
# #     try:
# #         tweets = scraper.get_tweets(user, mode="user", number=40)
# #         print(tweets)  # Assuming each page returns 10 tweets
# #         for page_tweets in tweets:
# #             for status in page_tweets:
# #                 print(status)
# #                 tw = preproc(status)
# #                 if " " not in tw:  # Checking if tw contains at least one space character
# #                     tw = "blank"
...
#         print("Returned data structure:", tweets)
        
#         if isinstance(tweets, dict) and 'tweets' in tweets:
#             tweets_list = tweets['tweets']
#             for tweet in tweets_list:
#                 print(tweet)  # Debug: Print each tweet
#                 tw = preproc(tweet)
#                 if " " not in tw:  # Checking if tw contains at least one space character
#                     tw = "blank"
#                 csvWriter.writerow([tw])
#         else:
#             print("Unexpected data structure:", tweets)
#     except Exception as e:
#         print("Failed to run the command on that user, Skipping...")
#         print(e)
#     csvFile.close()
...
# # consumer_key = '2LqQQXmEArurcZUYNQYc3IWpv'
# # consumer_secret = 'Yjn97W5BwCEd9GdO4miBM0tMvrmu3k081BM8vtMlvkbBpNn8CB'
# # access_token = '1302202863230136321-7xE7P4SOUgGfu3HE0ErAEf7dsCcsbr'
# # access_token_secret = 'JfvEfJ6SF0t2AJjRYWtofLS3r8ERB09sjzDGjP12CKL5Y'
...
# def twits(handle):
#     try:
#         tweets = scraper.get_tweets(handle, mode="user", number=1000)
#         tweet_texts = [tweet.text for tweet in tweets]
#         return tweet_texts
#     except Exception as e:
#         print("Failed to retrieve tweets for user:", handle)
#         print(e)
...
def preproc(s):
    ...
    stop_words_set = set(stopwords.words('english'))
    filtered_tokens = [w for w in tokens if w.lower() not in stop_words_set]
    stemmed_tokens = [SnowballStemmer("english").stem(w) for w in filtered_tokens]
    return ' '.join(stemmed_tokens)

def resource_path(relative_path):
    return os.path.join(os.getcwd(), relative_path)

# Fetch tweets from the specified handle
def twits(handle):
    try:
        tweets = scraper.get_tweets(handle, mode="user", number=1000)
...
# Write preprocessed tweets to CSV
def getTweets(handle):
    csv_file_path = resource_path('Resource_Images/user.csv')
    with open(csv_file_path, 'w', newline='') as csvFile:
        csvWriter = csv.writer(csvFile)
        try:
            tweets = scraper.get_tweets(handle, mode="user", number=40)
            if isinstance(tweets, dict) and 'tweets' in tweets:
                tweets_list = tweets['tweets']
                for tweet in tweets_list:
                    tweet_text = tweet.get('text', '')
                    if tweet_text:
                        preprocessed_text = pre_process(tweet_text)
                        if " " not in preprocessed_text:
                            preprocessed_text = "blank"
                        csvWriter.writerow([preprocessed_text])
                    else:
                        print("No text found in tweet:", tweet)
            else:
                print("Unexpected data structure:", tweets)
        except Exception as e:
            print("Failed to run the command on that user, Skipping...")
...
# Predict personality based on tweets
def twit(handle):
    getTweets(handle)
    with open(resource_path('Resource_Images/user.csv'), 'rt') as f:
        csvReader = csv.reader(f)
    ...
    with open(resource_path('Resource_Images/newfrequency300.csv'), 'rt') as f:
        csvReader = csv.reader(f)
        mydict = {rows[1].lower(): int(rows[0]) for rows in csvReader}

    vectorizer = TfidfVectorizer(vocabulary=mydict, min_df=1)
    x = vectorizer.fit_transform(tweetList).toarray()
    df = pd.DataFrame(x)
    # Load models using the old version of scikit-learn
    ...
    # Save models to in-memory objects (using the same version)
    models = {
      'model_IE': model_IE,
      'model_SN': model_SN,
      'model_TF': model_TF,
    ...
    # Optionally, you can save these models to disk with a different filename for later use
    with open('all_models_old.pkl', 'wb') as f:
      pickle.dump(models, f)
    

    model_IE = models['model_IE']
    model_SN = models['model_SN']
    model_TF = models['model_TF']
    model_PJ = models['model_PJ']

# Save models using the new version of scikit-learn
    with open('BNIEFinal_new.sav', 'wb') as f:
      pickle.dump(model_IE, f)

    with open('BNSNFinal_new.sav', 'wb') as f:
     pickle.dump(model_SN, f)

    with open('BNTFFinal_new.sav', 'wb') as f:
      pickle.dump(model_TF, f)

    with open('BNPJFinal_new.sav', 'wb') as f:
      pickle.dump(model_PJ, f)
...
class MyWindow:
    ...
    def home1(self):
        ...
        self.D_lbl0 = ttk.Label(newwin13, text='MBTI Personality Based Job Recommender Using Twitter Data ',
                                font=("Arial", 15))
...
window.title("MBTI Personality Predictor")
mywin = MyWindow(window)
window.geometry("400x300")
window.iconphoto(True, tk.PhotoImage(file=resource_path('Resource_Images/mbti_logo.png')))
window.mainloop()
